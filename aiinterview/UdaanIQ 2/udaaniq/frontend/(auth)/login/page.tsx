...
export default function LoginPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (res?.token) {
        // Store token for client-side fetches
        localStorage.setItem("auth_token", res.token);
        // Also set a client-readable cookie for middleware guard
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-white to-slate-50">
      <div className="w-full max-w-md rounded-xl bg-white border border-slate-200 p-8 shadow-sm">
        <div className="mb-6">
          <div className="text-sm uppercase tracking-wider text-slate-500">Sign in</div>
          <h1 className="text-2xl font-semibold mt-1 text-slate-900">Welcome back</h1>
        </div>
